        ...
        if (!(cell.classList.contains("disabled") || cell.classList.contains("empty"))) {
          if (true) {
            cell.addEventListener("mouseenter", function () {
              // Hoang: CO bien _mode.NormalMode, sao khong su dung?
          ...
          cell.addEventListener("click", function (e) {
            ...
            //Selected Mode: On  
            // Hoang: CO bien _mode, sao khong su dung?     
...
//Hoang: De o tren cung hoac duoi cung de de doc flow
//Hoang: de o trong calendarWidgets.forEach khong dc? Sao phai tach ra 2 cho?
window.onload = function () {
  ...
  // small
  if(calendarContentWidth < 800 || calendarContentHeight < 400){
  ...
  if (toolBar.checked) {
    ...
    if (checkmode == "Selected Mode") {
      ...
      if (toolBar.value === 'Yesterday') {
        ...
        clickFullDate = new Date(); // start
        clickFullDate.setHours(0, 0, 0, 0);
        clickFullDate.setDate(clickFullDate.getDate() - 1);
        toolFullDate = new Date();  // end
        toolFullDate.setHours(0, 0, 0, 0);
        toolFullDate.setDate(toolFullDate.getDate() - 1);
        
        console.log("toolFullDate", toolFullDate);
        highlightCellRange(toolFullDate, calendar.closest(".dual-calendar"));
      }
      if (toolBar.value === 'rad_weekBefore') {
        // Hoang: fix select opetation
        clickFullDate.setDate(clickFullDate.getDate() - 0);
        toolFullDate.setMonth(clickFullDate.getMonth());
        toolFullDate.setFullYear(clickFullDate.getFullYear());
      ...
      if (toolBar.value === 'rad_monthBefore') {
        // Hoang: fix select opetation
        clickFullDate.setDate(clickFullDate.getDate());
        toolFullDate.setMonth(clickFullDate.getMonth());
        toolFullDate.setFullYear(clickFullDate.getFullYear());
...
function eventClickButton() {
  ...
  // const btnReturn = document.querySelector(".btn_Cancel");
